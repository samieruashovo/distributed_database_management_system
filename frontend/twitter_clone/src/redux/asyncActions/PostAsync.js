...
export const post_detail = (uuid) => async (dispatch) => {
  ...
  try {
    ...
    if (localStorage.getItem("access")) {
      ...
      // console.log("asdfdsfd post")
      dispatch(postDetail(res.data));
    }
    await axiosInstance.get(`${url}community/detail/${uuid}`);
  } catch (err) {
    console.log(err+"asdf post");
  }
};

export const post_bookmark_list = () => async (dispatch) => {
  dispatch(setLoading(true));
  try {
...
export const post_specific_user = (username) => async (dispatch) => {
  dispatch(setLoading(true));
  try {
