...
export const post_comments = (uuid) => async (dispatch) => {

  dispatch(setLoading(true));
  try {
    console.log('running post comments')
    if (localStorage.getItem("access")) {
      const res = await axiosInstance.get(`${url}community/comments/list/${uuid}`);
      dispatch(setLoading(false));
      dispatch(setMeta(res.data.meta));
      console.log(res.data)
      console.log("post commenst here")
...
export const addComment =
  ...
  async (dispatch) => {
    dispatch(commentUploading(true));
    try {
      console.log("post running addPostComment")
        const jsonData = localStorage.getItem("userData");
    const dataObject = JSON.parse(jsonData);
    console.log("gender"+dataObject.data.gender)
        const res = await axiosInstance.post(`community/comments/list/${uuid}`, {
          body,
          gender: dataObject.data.gender,
          username: dataObject.data.username,
          post_uuid: uuid
        ...
        console.log(res.data)
        dispatch(postCommentUploading(false));
        dispatch(postCommentAdded(res.data));

        dispatch(setMessage("Comment Added on!"));
      // }
    } catch (err) {
      dispatch(commentUploading(false));
      // dispatch(setMessage("Something went Wrong !"));
